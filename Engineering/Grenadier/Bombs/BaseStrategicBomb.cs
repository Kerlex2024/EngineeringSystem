...
{
	public abstract class BaseStrategicBomb : BasePotion
	{
		...
		public BaseStrategicBomb(PotionEffect effect)
			: base(0xF0D, effect)
		{ EnsureItemHue(); }

		public BaseStrategicBomb(Serial serial)
			: base(serial)
		{ EnsureItemHue(); }
		public virtual int ExplosionHue
		{
			get { return Server.Custom.Engineering.Grenadier.GrenadierConfig.StrategicHue; }
		}
		...
		// Force the item's bottle hue to match config
		...
		public void Explode(Mobile from, bool direct, Point3D loc, Map map)
		{
			...
			foreach (object o in eable)
			{
				...
				else if (o is BaseStrategicBomb && o != this)
				{
					toExplode.Add(o);
				}
			}
			...
			for (int i = 0; i < toExplode.Count; ++i)
			{
				...
				else if (o is BaseStrategicBomb)
				{
					BaseStrategicBomb pot = (BaseStrategicBomb)o;
		...
		{
			private readonly BaseStrategicBomb m_Potion;

			public ThrowTarget(BaseStrategicBomb potion)
				: base(12, true, TargetFlags.None)
			{
				m_Potion = potion;
			}

			public BaseStrategicBomb Potion { get { return m_Potion; } }
